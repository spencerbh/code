...
// Notes
console.log('Notice that on their own Alice, Bob, and Carol just have keys. \
            In order to have accounts that can hold tokens they need to connect to a network. \
            The Paypal network is one such network, but Bitcoin and Ethereum are also networks. \
            The state of the network is what determines account balances, so how the network operates \
            is very important to users.');


console.log('Btw, you might notice that the public key is different than the address. \
            This is because Ethereum addresses are generated from public keys, but are not exactly the same thing. \
            Here\'s more info on that process: \
...
// it this intermediate step redundant? hashedMessageFromAlice serves what purpose?
// purposed served in bob verifying the message
...
console.log('To do this we need to verify the message signature and the message hash to \
            see if they return Alice\'s address');
